...
int N,K,C,BT;         //Initialising the variables
...
//mixed technique to compute the C matrix
void* mixed_technique(void* arg){
    intermediary* inner = (intermediary*)arg;

    //For Bounded Threads
    if(inner->thread_no-1 < BT){
        auto start = chrono::steady_clock::now();
        cpu_set_t mask;                         //Timer starts
        CPU_ZERO(&mask);                        //initialsing the CPU SET
        int cpu_no = (inner->thread_no-1)*C/K;  //Emptying the set
        ...
        for(int i = inner->thread_no-1; i < N; i += K){
            for(int j = 0; j < N; j++){
        ...
        time_array[inner->thread_no-1] = iteration_time;  //Storing the execution time in the time_array
    }
    ...
    //For Normal threads
        ...
        std::chrono::duration<double> duration = end - start;
        double iteration_time = duration.count();
        time_array[inner->thread_no-1] = iteration_time;
    }
...
int main(){
    ...
    for(int i = 0; i < K; i++){
        bridge[i]->thread_no = i+1;
        if(pthread_create(thread_ptr + i, NULL, mixed_technique, bridge[i])){
