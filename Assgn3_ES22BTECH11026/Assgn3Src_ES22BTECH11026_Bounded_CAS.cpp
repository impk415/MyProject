...
bool waiting[32];  //waiting data structutre initialised to false, one for each thread.
...
intermediary** bridge;        //bridge variable points to an array of structures
                            //ACTS AS A BRIDGE BY CONTAINING ALL THE ARGUMENTS TO BE PASSED TO THE function dynamic_Bounded_CAS
...
//dynamic_Bounded_CAS technique to compute the C matrix
void* dynamic_Bounded_CAS(void* arg){
    intermediary* inner = (intermediary*)arg;
    int local_row_counter = 0;
    while(true){
        waiting[inner->thread_no] = true;
        int key = 1;
        int expected = 0;
        while(waiting[inner->thread_no] && key==1){
            key = !(flag.compare_exchange_strong(expected, 1));
        }
        waiting[inner->thread_no] = false;
        ...
        int j = (inner->thread_no + 1)%K;       //CHECKING whether the next thread wants to enter the critical section
        while((j != inner->thread_no)  && !waiting[j]){
            j = (j + 1) % K;
        }

        if(j == inner->thread_no){
            flag = 0;
        }
        else{
            waiting[j] = false;
...
int main(){
    ...
    fp = fopen("output_Bounded_CAS.txt", "w");
    ...
    for(int i = 0; i < K; i++){
        ...
        if(pthread_create(thread_ptr + i, NULL, dynamic_Bounded_CAS, bridge[i])){
