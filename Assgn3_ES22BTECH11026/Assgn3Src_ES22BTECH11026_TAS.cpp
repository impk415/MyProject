...
intermediary** bridge;        //bridge variable points to an array of structures
                            //ACTS AS A BRIDGE BY CONTAINING ALL THE ARGUMENTS TO BE PASSED TO THE function dynamic_TAS
...
//DYNAMIC_TAS technique to compute the C matrix
void* dynamic_TAS(void* arg){
...
int main(){
    ...
    for(int i = 0; i < K; i++){
        ...
        if(pthread_create(thread_ptr + i, NULL, dynamic_TAS, bridge[i])){
